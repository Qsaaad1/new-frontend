...
function AdminVolunteers() {
  ...
  useEffect(() => {
    ...
    const filtered = collections.filter((collection) =>
      collection.First_Name.toLowerCase().includes(searchTerm.toLowerCase())
    );
  ...
  const fetchCollections = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_RENDER_URL}/volunteer-collections`
      ); // Update the URL
  ...
  //   const data = user.First_Name + user.Last_name
    ...
    <>
      ...
      <div className="flex min-h-screen">
        ...
        <div className={`w-full bg-gray-200 border-r border-gray-300 p-4`}>
          <h2 className="font-bold text-lg mb-4">Volunteers Profile</h2>
            ...
            <ul>
              {filteredCollections.map((collection, index) => (
                <li
                  key={index}
                >
                  <Link to={{ pathname: `/volunteer-profile/${collection.First_Name}-${collection.Last_name}`}}
                   state={{ First_Name: collection.First_Name, 
                   Last_Name: collection.Last_name,
                   Gender: collection.Gender,
                   Countrie: collection.Countries,
                   Citie: collection.Cities,
